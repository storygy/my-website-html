...
// 在 App.tsx 中，和其他函数一起添加（比如 handlePreview、handleShare 等附近）
const handleTogglePublic = useCallback(async (app: AppItem) => {
  try {
    ...
    if (user) {
      // 这里调用你的云存储更新函数
      // 如果你有 updateAppPublicStatus 函数，可以这样用：
      // await updateAppPublicStatus(app.id, newPublicStatus);
      
      // 或者简单点，直接同步所有数据
      if (syncToCloud) {
        await syncToCloud(apps.map(a => 
    ...
    // 可选：显示成功提示
    // toast.success(`应用已${newPublicStatus ? '公开' : '私密'}`);
  } catch (error) {
...
}, [apps, user, updateApp, syncToCloud]);
